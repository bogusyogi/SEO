<!-- Updated: 2026-07-04 -->
# Core Web Vitals Thresholds (July 2026)
...
- **March 2026 core update — reported composite CWV scoring** (SEO-industry reporting, MEDIUM
  confidence, not Google-confirmed): LCP+INP+CLS allegedly evaluated as an aggregate — passing all
  three compounds the benefit, failing even one compounds the penalty. Practical stance: treat
  "pass all three" as the target (that was already the goal); do NOT cite "composite scoring" as a
  Google mechanic to clients without re-verifying against official docs at audit time.
- **2026 update timeline** (for "did an update hit me" diagnosis): March 2026 core (Mar 27–Apr 8,
  very high volatility — ~80% of top-3 results shifted), May 2026 core (May 21–Jun 2), June 2026
  spam update (second spam update of the year). Check dates against traffic-drop onset before
  diagnosing site-side causes.
